...
def download(s, img):
    ...
    for i in range(0, len(img)):
        try:
            ...
            mPos = -1
        if(mPos >= 0):
            print("Find a GIF，loading to 'temp'，please combined them into video later by yourself.")
...
def download_gif(s, img):
        ...
        while (ifloop == 1):
            try:
                ...
                if (status == 404):
                    ...
                    img2.append(temp)
                    # 添加到图片ID列表
                    print("Loading the %s image of GIF..." % nummber)
                    nummber = nummber + 1
                    j = j + 1
            except:
                j = j
    ...
    name2change = str(name2change).replace('\']', '')
    name2change = str(name2change).replace('\',\'', '')
    pic2video(file2change, name2change)
    print('------------------------------------------------------------------------------')
